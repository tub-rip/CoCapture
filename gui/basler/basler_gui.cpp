...
namespace rcg::gui::basler {
    ...
    BaslerGui::BaslerGui(cams::basler::BaslerCamera* basler_camera, int id) {
        // Camera
        basler_camera_ = basler_camera;

        // Information
        identifier_ = "basler" + std::to_string(id);
        serial_number_ = basler_camera_->GetSerialNumber();
        model_name_ = basler_camera_->GetModelName();
        vendor_name_ = basler_camera_->GetVendorName();

        // Image Frame
        image_frame_width_ = basler_camera_->GetImageFrameWidth();
        image_frame_height_ = basler_camera_->GetImageFrameHeight();

        // Settings
        exposure_time_ = basler_camera_->GetExposureTime();
        trigger_mode_ = basler_camera->GetTriggerMode();
        reverse_x_ = basler_camera_->GetReverseX();
    ...
    void BaslerGui::Show(bool* show) {
        // Display image frame
        if(show_visualization_) {
            basler_camera_->OutputFrame(image_frame_);
        ...
        if(show == nullptr || *(show)) {
            ...
            ImGui::BeginDisabled(basler_camera_->IsStarted());
            if(ImGui::Button(button_start_label_.c_str())) {
                basler_camera_->Start();
            }
            ImGui::EndDisabled();
            ImGui::SameLine();
            ImGui::BeginDisabled(!basler_camera_->IsStarted() || basler_camera_->IsRecording());
            ...
            if(basler_camera_->IsStarted()) {
                if(ImGui::CollapsingHeader(collapsing_header_general_label_.c_str())) {
                    ...
                    if(ImGui::Checkbox(checkbox_trigger_mode_label_.c_str(), &trigger_mode_)) {
                        basler_camera_->SetTriggerMode(trigger_mode_);
                    }
                    ImGui::Spacing();
                    ...
                    if(ImGui::Checkbox(checkbox_reverse_x_label_.c_str(), &reverse_x_)) {
                        basler_camera_->SetReverseX(reverse_x_);
                    }
                    ImGui::Spacing();
                    ImGui::EndDisabled();
                    ...
                    if(ImGui::Button(button_start_recording_label_.c_str())) {
                        ...
                        basler_camera_->StartRecording(output_dir_.c_str());
                    }
                    ImGui::EndDisabled();
                    ImGui::SameLine();
                    ImGui::BeginDisabled(!basler_camera_->IsRecording());
                    if(ImGui::Button(button_stop_recording_label_.c_str())) {
                        basler_camera_->StopRecording();
