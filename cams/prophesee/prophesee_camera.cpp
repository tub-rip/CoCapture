...
namespace rcg::cams::prophesee {
    ...
    std::map<std::string, int> PropheseeCamera::AnalyzeRecording(const char* output_dir) {
        std::string output_dir_str = std::string {output_dir};
        if(output_dir_str.back() == '/') { output_dir_str.pop_back(); }

        std::map<std::string, int> recording_information;

        std::fstream trigger_events_file;
        trigger_events_file.open((output_dir_str + "/trigger_events.txt").c_str(), std::ios::in);
        ...
        recording_information.insert({"trigger_events_count", trigger_events_count});
